	...
	context 'Requesting a journey time between origin and destination' do
		...
		context 'generating calls to API' do
			...
			it 'should make a request to the Distance Matrix API' do
				...
				expect(JourneyTimeCalculator).to receive(:fetch_data).with(query_string)
			...
			it 'a request response should include journey time' do
				...
				expect(JourneyTimeCalculator.fetch_data(url)['rows'][0]['elements'][0]).to have_key("duration")
				expect(JourneyTimeCalculator.fetch_data(url)).to include("status" => "OK")
			end
		...
		context 'retrieving journey times' do
			...
			it 'should return a time given an origin and a destination' do
				expect(JourneyTimeCalculator.drive_time_between(origin, destination)).to be_within(10).of(20)
			end
